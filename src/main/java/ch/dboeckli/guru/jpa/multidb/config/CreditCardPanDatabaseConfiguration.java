...
@Configuration
@EnableJpaRepositories(basePackageClasses = CreditCardPanRepository.class,
        entityManagerFactoryRef = "entityManagerFactory", transactionManagerRef = "panTransactionManager")
public class CreditCardPanDatabaseConfiguration {
    ...
    public LocalContainerEntityManagerFactoryBean panEntityManagerFactory(
        ...
        return builder.dataSource(panDataSource).packages(CreditCardPan.class).persistenceUnit("pan").build();
    }

    @Primary
    @Bean
    public PlatformTransactionManager panTransactionManager(
            @Qualifier("entityManagerFactory") LocalContainerEntityManagerFactoryBean panEntityManagerFactory) {
